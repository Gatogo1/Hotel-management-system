...
if(isset($_POST["btn_update"]))
{
    ...
    $q1="UPDATE `drinks_invoice` SET `room_id`='$room_id',`cust_id`='$cust_id',`cust_name`='$cust_name',`cust_email`='$cust_email',`cust_contact`='$cust_contact',`cust_address`='$cust_address',`total_amount`='$total_amount' WHERE `id`='".$_GET['id']."'";
    //$q2=$conn->query($q1);
    if ($conn->query($q1) === TRUE) {
        $sql = "DELETE FROM `drinks_invoice_items` WHERE drinks_invoice_id='".$_GET["id"]."'";
        $res = $conn->query($sql) ;
        for ($i=0; $i <count($drink_id) ; $i++) { 
    $sql = "INSERT INTO `drinks_invoice_items`(`drinks_invoice_id`, `drink_id`, `price`, `qty`, `subtotal`) VALUES ('".$_GET["id"]."', '$drink_id[$i]','$price[$i]','$qty[$i]','$subtotal[$i]')";
   $conn->query($sql);
  }
         ...
         <div class="page-wrapper">
            ...
            <div class="row page-titles">
                <div class="col-md-5 align-self-center">
                    <h3 class="text-primary">Edit Drink Invoice Payment</h3> </div>
                <div class="col-md-7 align-self-center">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="javascript:void(0)">Home</a></li>
                        <li class="breadcrumb-item active">Edit Drink Invoice Payment</li>
                    </ol>
                </div>
            </div>
            <!-- End Bread crumb -->     
                            ...
                            <div class="card-body">
                                <div class="form-validation">
                                    ...
                                    <form class="form-valide"  method="post" enctype="multipart/form-data">
                                       ...
                                       <div class="mydiv">
                                        <?php 
                                        $item_q="SELECT * FROM `drinks_invoice_items` WHERE drinks_invoice_id='".$_GET["id"]."'";
                                        ...
                                        <div class="form-group row control-group after-add-more subdiv">
                                            ...
                                            <select class="form-control" name="drink_id[]" required>
                                                ...
                                                while($drink = $drink_res->fetch_assoc()) { ?>
                                                    <option value="<?php echo $drink['id']; ?>" data-price="<?php echo $drink['price']; ?>" <?php if($item['drink_id']==$drink['id']){ echo "selected"; } ?>><?php echo $drink['item_name']; ?></option>
                                                <?php } ?>
                                            </select>
                                            </div>   
                                            <div class="col-sm-2">
