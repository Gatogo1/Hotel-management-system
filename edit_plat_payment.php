...
if(isset($_POST["btn_update"]))
{
    ...
    $q1="UPDATE `plats_invoice` SET `room_id`='$room_id',`cust_id`='$cust_id',`cust_name`='$cust_name',`cust_email`='$cust_email',`cust_contact`='$cust_contact',`cust_address`='$cust_address',`total_amount`='$total_amount' WHERE `id`='".$_GET['id']."'";
    //$q2=$conn->query($q1);
    if ($conn->query($q1) === TRUE) {
        $sql = "DELETE FROM `plats_invoice_items` WHERE plats_invoice_id='".$_GET["id"]."'";
        $res = $conn->query($sql) ;
        for ($i=0; $i <count($plats_id) ; $i++) { 
    $sql = "INSERT INTO `plats_invoice_items`(`plats_invoice_id`, `plats_id`, `price`, `qty`, `subtotal`) VALUES ('".$_GET["id"]."', '$plats_id[$i]','$price[$i]','$qty[$i]','$subtotal[$i]')";
   $conn->query($sql);
  }
...
?>
<?php
$que="SELECT * FROM `plats_invoice` WHERE id='".$_GET["id"]."'";
         ...
         <div class="page-wrapper">
            ...
            <div class="row page-titles">
                <div class="col-md-5 align-self-center">
                    <h3 class="text-primary">Edit Food Invoice Payment</h3> </div>
                <div class="col-md-7 align-self-center">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="javascript:void(0)">Home</a></li>
                        <li class="breadcrumb-item active">Edit Food Invoice Payment</li>
                    </ol>
                </div>
            </div>
            <!-- End Bread crumb -->     
                            ...
                            <div class="card-body">
                                <div class="form-validation">
                                    ...
                                    <form class="form-valide"  method="post" enctype="multipart/form-data">
                                       ...
                                       <div class="mydiv">
                                        <?php 
                                        $item_q="SELECT * FROM `plats_invoice_items` WHERE plats_invoice_id='".$_GET["id"]."'";
                                        ...
                                        <div class="form-group row control-group after-add-more subdiv">
                                            <div class="col-sm-3">
                                            <select class="form-control" name="plats_id[]" required>
                                                <option value="">Select Food</option>
                                                <?php
                                                $drink_sql = "SELECT * FROM `plats` WHERE status=0";
                                                $drink_res = $conn->query($drink_sql);
                                                while($drink = $drink_res->fetch_assoc()) { ?>
                                                    <option value="<?php echo $drink['id']; ?>" data-price="<?php echo $drink['price']; ?>" <?php if($item['plats_id']==$drink['id']){ echo "selected"; } ?>><?php echo $drink['item_name']; ?></option>
                                                <?php } ?>
                                            </select>
                                            </div>   
                                            <div class="col-sm-2">
...
<div class="popup popup--icon -error js_error-popup error_dates">
  ...
  <div class="popup__content">
    ...
    <p id="dates" class="text-danger">Oops Food Already In List!!</p>
    <p>
...
<script type="text/javascript">
    $(document).ready(function(){      
       ...
       $('div.mydiv').on("change",'select[name^="plats_id"]',function(event){
 ...
 $('#total_amount').val(tot_commi);
       ...
       function check_product(drink_id) {
        var flag=0;
        $("div.mydiv").find('select[name^="plats_id"]').each(function () {
